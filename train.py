...
if __name__ == '__main__':
  ...
  dataloader = DataLoader(musdb_dataset, batch_size=batch_size, shuffle=True,
                          num_workers = 12, prefetch_factor = 8, pin_memory=True)
  print("Dataset loaded...")
  
  # Models 
  model = steminator.Steminator(hparams.input_channels, hparams.num_of_filters, hparams.output_channels)
  model.cuda()
  ...
  for epoch in range(0, hparams.number_of_epochs):
      ...
      for i, batch in enumerate(dataloader):
          ...
          # Append zeros to the 
          zeros_vector = torch.zeros(x_.size(0), x_.size(1), x_.size(2), 1).cuda()
